...
export class DepreciationTableComponent {
    ...
    constructor(/*private model: StaticModel, */private store$: Store<fromRoot.State>) {
      this.entityState = this.store$.select(fromRoot.depreciationEntityState);
      this.valueObjectState = this.store$.select(fromRoot.depreciationValueObjectState);
    }
